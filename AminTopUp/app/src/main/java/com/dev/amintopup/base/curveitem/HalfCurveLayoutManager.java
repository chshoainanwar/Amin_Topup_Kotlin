...
public class HalfCurveLayoutManager extends CenterEdgeItemsRecyclerView.ChildLayoutManager {
    ...
    public void updateChild(View child, CenterEdgeItemsRecyclerView parent) {
        ...
        if (yRelativeToCenterOffset > 0.49 && yRelativeToCenterOffset < 0.51) {
            img.setColorFilter(ContextCompat.getColor(img.getContext(), R.color.orangeClr), android.graphics.PorterDuff.Mode.SRC_IN);
//            params.height = img.getContext().getResources().getDimensionPixelOffset(R.dimen._95);
//            params.width = img.getContext().getResources().getDimensionPixelOffset(R.dimen._95);
...
//            child.animate().scaleX(1.2f).scaleY(1.2f).translationX(-70f).setInterpolator(new AccelerateDecelerateInterpolator()).setDuration(250).setStartDelay(50).start();
            child.animate()
                    .scaleX(1.125f)
                    .scaleY(1.125f)
                    .translationX(-60F)
                    .setInterpolator(new AccelerateDecelerateInterpolator()).setDuration(250).setStartDelay(50).start();
        } else {
            img.setColorFilter(ContextCompat.getColor(img.getContext(), R.color.unselectedClr), android.graphics.PorterDuff.Mode.SRC_IN);
//            params.height = img.getContext().getResources().getDimensionPixelOffset(R.dimen._63);
//            params.width = img.getContext().getResources().getDimensionPixelOffset(R.dimen._63);
//            img.setLayoutParams(params);
